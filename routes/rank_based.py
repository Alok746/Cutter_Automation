    ...
    def compare_ranked():
        ...
        if max_rank > actual_max:
            return render_template(
                ...
                sheet=sheet,
                columns=all_columns,
                error_message=f"Top {max_rank} not available. Max rank in data is {actual_max}."
            )

        rank_labels = [f"Rank {i}" for i in range(1, max_rank + 1)]

        # ✅ Fix: Calculate total unique respondents across any of the ranked columns
        ...
        # ✅ Corrected: % overall = row_total / total_respondents
        percent_matrix = []
